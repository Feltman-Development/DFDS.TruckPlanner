...
public static class ReflectionUtility
{
    ...
    /// </summary>
    public static IEnumerable<Type> GetGenericTypes(Type genericType, Assembly assembly)
        => assembly.GetTypes().Where(t => t.BaseType is {IsGenericType: true} && t.BaseType.GetGenericTypeDefinition() == genericType);
    ...
    /// </summary>
    public static object? GetField(object fromObject, string property) => fromObject.GetType().GetField(property, MemberAccess)?.GetValue(fromObject);
    ...
    public static object CallMethod(object instance, string method, params object[]? parms)
    {
        ...
        if (parms == null) return CallMethod(instance, method, parameterTypes, parms!);
    ...
    public static object CallMethodQualified(object parent, string method, object[] parameters)
    {
        while (true)
        {
            ...
            // Walk the . syntax
            var main = method[..lnAt];
            var subs = method[(lnAt + 1)..];
    ...
    {
        if (property is "this" or "me") return parent;
        ...
        {
            pureProperty = property?[..property.IndexOf("[", StringComparison.Ordinal)];
            indexes = property?[property.IndexOf("[", StringComparison.Ordinal)..]!;
            isArrayOrCollection = true;
        }
        var member = parent.GetType().GetMember(pureProperty!, MemberAccess)[0];
        var result = member.MemberType == MemberTypes.Property ? ((PropertyInfo)member).GetValue(parent, null) : ((FieldInfo)member).GetValue(parent);
        if (!isArrayOrCollection) return result!;

        indexes = indexes.Replace("[", string.Empty).Replace("]", string.Empty);
        if (result is not Array)
        {
            if (result is not ICollection) return result!;
            ...
            TryParse(indexes, out var index);
    ...
    public static IEnumerable<Type> FindDerivedTypes(Assembly assembly, Type baseType)
    {
        ...
        return from type in assembly.DefinedTypes where isClass
